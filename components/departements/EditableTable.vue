...
import { usedepartementStore } from '@/stores/apps/departement';
import { usepaysStore } from '@/stores/apps/pays';


const store = usedepartementStore();
const storePays = usepaysStore();
onMounted(() => {
    store.fetchdepartements();
    storePays.fetchpayss();
});
const getdepartements: any = computed(() => {
    return store.departements;
});
...
//const departement = ref(['Sénégal', 'France']);

const listdepartements = ref(getdepartements)
...
const filteredList = computed(() => {
    return listdepartements.value.filter((user: any) => {
...
function editItem(item: any) {
    console.log(listdepartements.value.indexOf(item))
    editedIndex.value = 1;
    editedItem.value = Object.assign({}, item);
    dialog.value = true;
}
function deleteItem(item: any) {
    confirm('Are you sure you want to delete this item?') && store.deletedepartements(item);
}
...
function save() {
    ...
    if (editedIndex.value ==1) {
        store.updatedepartements(editedItem.value)
    } else {
        store.adddepartements(editedItem.value)       
    }
    editedIndex.value = 0;
    close();
}
    ...
    <v-table class="mt-5">
        <thead>
            <tr>
                ...
                <th class="text-subtitle-1 font-weight-semibold">Region</th>
            ...
            <tr v-for="item in filteredList" :key="item.node.id">
                ...
                <!-- <td>
                    <div class="d-flex align-center py-4">
                        ...
                        <div class="ml-5">
                            <h4 class="text-h6 text-no-wrap">{{ item.node.departement && item.node.departement.name || ''}}</h4>                           
                        </div>
                    </div>
                </td>-->
                <td> 
