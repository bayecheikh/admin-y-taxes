...
import departement from '@/_mockApis/apps/departements';
...
const departements = ref(['Dakar', 'Pikine','Thies']);
const entites = ref(['Entité 1']);
const organes = ref(['Organe 1']);
const desserts = ref(departement);
...
const formTitle = computed(() => {
    return editedIndex.value === -1 ? 'Ajouter un departement' : 'Modifier un departement';
});
</script>
<template>
    <v-row>
        <v-col cols="12" lg="4" md="6">
            <v-text-field density="compact" v-model="search" label="Rechercher un departement" hide-details variant="outlined"></v-text-field>
        </v-col>
        <v-col cols="12" lg="8" md="6" class="text-sm-right">
            <v-dialog v-model="dialog" max-width="500">
                <template v-slot:activator="{ props }">
                    <v-btn color="primary" v-bind="props" flat class="ml-auto">
                        <v-icon class="mr-2">mdi-account-multiple-plus</v-icon>Ajouter un departement
                    </v-btn>
                </template>
                <v-card>
